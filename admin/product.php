                        ...
                        LEFT JOIN category ON products.categoryid = category.categoryid ORDER BY `productid` DESC"); ?> 
                            <?php while ($row = $results->fetch_assoc()) { ?>
                                <tr>
                                    ...
                                    <td><span class='badge <?php echo ($row["status"] == 'Available' ? 'bg-success' : 'bg-danger') ?>'> <?php echo $row["status"]; ?> </span></td>
                                    <td><?php echo $row['categoryname'];?></td>
                                    <td><?php echo $row['dateadded'];?></td>
                                    <td>
                                        <a href="updateProduct.php?updateProductById=<?php echo $row['productid'];?>" class="btn btn-warning btn-sm shadow rounded me-md-2"><i class="fas fa-edit me-2"></i>Edit</a>
                                        <a href="functions/processproduct.php?deleteProductById=<?php echo $row['productid'];?>" class="btn btn-danger btn-sm shadow rounded me-md-2" id="btn-delProd"><i class="fas fa-trash-alt me-2"></i>Delete</a>
                                    </td>
                                </tr>
                            <?php } ?>
                        </tbody>
                            ...
                            <form action="functions/processproduct.php" method="post">
                                <div class="modal-body">
                                    <div class="form-floating mb-3">
                                        <input class="form-control form-control-xl mb-2" type="text" name="productname"id="floatingInput" placeholder="Product Name" required>
                                        <label for="floatingInput">Product Name</label>
                                    </div>
                                    <div class="form-floating mb-3">
                                        <input class="form-control form-control-xl mb-2" type="text" name="price" onblur="formatAmount(this)" id="floatingInput" placeholder="Price" required>
                                ...
                                <div class="modal-footer">
                                    ...
                                    <button type="submit" class="btn btn-dark shadow-lg rounded" name="addProduct"><i class="fas fa-solid fa-save"></i> Add product</button>
                                </div>                            
                            </form>
                    </div>
                </div>
